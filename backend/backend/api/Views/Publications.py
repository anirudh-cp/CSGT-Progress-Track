...
class PublicationsSinglePutApiView(APIView):
    ...
    def put(self, request, Emp_ID, Type, *args, **kwargs):
        ...
        data = request.data
        queryData = model[Type].objects.filter(Emp_ID=Emp_ID, DOI=data['DOI'])
        if queryData.exists():
            data.pop('Type', None)
...
class PublicationsAllApiView(APIView):
    ...
    def get(self, request, Type, startDate, endDate, *args, **kwargs):
        ...
        queryData = (model[Type].objects.filter(Conference_startdate__gt=startDate, 
                                                Conference_startdate__lte=endDate) | 
                     model[Type].objects.filter(Conference_startdate__lte=startDate, 
                                                Conference_enddate__gte=startDate))
